...
import { GroupMessage, Message } from "@prisma/client";
...
import { DrawerWrapper } from "@/app/conversations/_components/DrawerWrapper";
...
export const SendGroupMessages: FC<SendMessagesProps> = ({
  currentMemberId,
  groupId,
}) => {
  ...
  const { mutate: sendGroupMessage } = useMutation({
    mutationFn: async ({
      value,
      fileUrl,
    }: {
      ...
      const { data } = await axios.post("/api/groupMessage", {
        value,
        fileUrl,
        groupId,
      });
    ...
    onSettled: () => {
      ...
      socket?.emit("new-group-message-settled", groupId);
    },
  });
  ...
  const handleKeyPress = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter" && !e.shiftKey && value.trim() !== "") {
      ...
      sendGroupMessage({ value, fileUrl });
    }
  };

  const handleSubmit = (value: string) => {
    if (value.trim() !== "") sendGroupMessage({ value, fileUrl });
  };
  return (
